defmodule Rex.Fun do
  ...
  defp quoted_expr(quoted, env) do
    ...
    quote do
      fn {data, prog} -> {[unquote(expr) | data], prog} end
    end
  end

  defmacro dequote do
    quote do
      fn {[{quoted, env} | data], prog} ->
  ...
  defp fun_expr({:/, _, [{name, _, nil}, arity]}) when is_atom(name) and arity > 0 do
    ...
    quote do
      fn {[unquote_splicing(Enum.reverse(vars)) | stack], program} ->
        {[unquote(name)(unquote_splicing(vars)) | stack], program}
      end
    end
  end
  ...
  defp fun_expr({:/, _, [{ref, _, []}, 0]}) do
    quote do
      fn {data, prog} -> {[unquote(ref)() | data], prog} end
    end
  end
  ...
  defp fun_expr({:/, _, [{ref, _, []}, arity]}) do
    vars = for i <- 0..arity-1, do: Macro.var(:"v#{i}", nil)
    quote do
      fn {[unquote_splicing(Enum.reverse(vars)) | stack], prog} ->
        {[unquote(ref)(unquote_splicing(vars)) | stack], prog}
      end
    end
  end
  ...
  defp fun_expr(expr = {:quote, _, _}) do
    ...
    quote do
      fn {data, prog} -> {[unquote(code) | data], prog} end
    end
  end
